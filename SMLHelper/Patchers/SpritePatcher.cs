namespace SMLHelper.V2.Patchers
{
    ...
    internal class SpritePatcher
    {
        ...
        private static Dictionary<SpriteManager.Group, Dictionary<string, UnityEngine.Sprite>> _groups;
        private static Dictionary<SpriteManager.Group, Dictionary<string, UnityEngine.Sprite>> Groups => _groups ?? (_groups = (Dictionary<SpriteManager.Group, Dictionary<string, UnityEngine.Sprite>>)groupsInfo.GetValue(null));

        private static Dictionary<string, Dictionary<string, UnityEngine.Sprite>> _atlases;
        private static Dictionary<string, Dictionary<string, UnityEngine.Sprite>> Atlases => _atlases ?? (_atlases = (Dictionary<string, Dictionary<string, UnityEngine.Sprite>>)atlasesInfo.GetValue(null));

        internal static void Patch()
        {
            foreach (SpriteManager.Group moddedGroup in ModSprite.ModSprites.Keys)
            {
                Dictionary<string, UnityEngine.Sprite> spriteGroup = GetSpriteGroup(moddedGroup);
                if (spriteGroup == null)
                    return;
        ...
        private static Dictionary<string, UnityEngine.Sprite> GetSpriteGroup(SpriteManager.Group groupKey)
        {
            if (groupsInfo != null)
            {
                return Groups[groupKey];
