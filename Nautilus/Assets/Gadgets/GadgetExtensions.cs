...
public static class GadgetExtensions
{
    ...
    extension(ICustomPrefab customPrefab)
    {
        /// <summary>
        ...
        public CraftingGadget SetRecipe(RecipeData recipeData)
        {
        ...
        /// <returns>A reference to the created <see cref="CraftingGadget"/> to continue the recipe settings on.</returns>
        public CraftingGadget SetRecipeFromJson(string filePath)
        {
        ...
        public ScanningGadget SetUnlock(TechType requiredForUnlock, int fragmentsToScan = 1)
        {
        ...
        /// <returns>A reference to the created <see cref="ScanningGadget"/> to continue the scanning settings on.</returns>
        public ScanningGadget CreateFragment(TechType blueprint, float scanTime, int fragmentsToScan = 1, string encyKey = null, bool destroyAfterScan = true, bool isFragment = true)
        {
            if (!customPrefab.TryGetGadget(out ScanningGadget scanningGadget))
        ...
        public ScanningGadget SetPdaGroupCategory(TechGroup group, TechCategory category)
        {
            if (!customPrefab.TryGetGadget(out ScanningGadget scanningGadget))
        ...
        /// <remarks>If the specified <paramref name="group"/> is a tech group that is present in the <see cref="uGUI_BuilderMenu.groups"/> list, this item will automatically
        /// become buildable. To avoid this, or make this item a buildable manually, use the <see cref="ScanningGadget.SetBuildable"/> method.</remarks>
        public ScanningGadget SetPdaGroupCategoryAfter(TechGroup group, TechCategory category, TechType target)
        {
            if (!customPrefab.TryGetGadget(out ScanningGadget scanningGadget))
                scanningGadget = customPrefab.AddGadget(new ScanningGadget(customPrefab, TechType.None));
        ...
        /// <remarks>If the specified <paramref name="group"/> is a tech group that is present in the <see cref="uGUI_BuilderMenu.groups"/> list, this item will automatically
        /// become buildable. To avoid this, or make this item a buildable manually, use the <see cref="ScanningGadget.SetBuildable"/> method.</remarks>
        public ScanningGadget SetPdaGroupCategoryBefore(TechGroup group, TechCategory category, TechType target)
        {
            if (!customPrefab.TryGetGadget(out ScanningGadget scanningGadget))
                scanningGadget = customPrefab.AddGadget(new ScanningGadget(customPrefab, TechType.None));
        ...
        public EquipmentGadget SetEquipment(EquipmentType equipmentType)
        {
        ...
        public UpgradeModuleGadget SetVehicleUpgradeModule(EquipmentType equipmentType = EquipmentType.VehicleModule, QuickSlotType slotType = QuickSlotType.Passive)
        {
        ...
        public FabricatorGadget CreateFabricator(out CraftTree.Type treeType)
        {
        ...
        public ICustomPrefab SetSpawns(params SpawnLocation[] spawnLocations)
        {
        ...
        /// <returns>A reference to this instance after the operation has completed.</returns>
        public ICustomPrefab SetSpawns(params LootDistributionData.BiomeData[] biomesToSpawnIn)
        {
        ...
        /// <param name="biomesToSpawnIn">The biomes to spawn in.</param>
        /// <returns>A reference to this instance after the operation has completed.</returns>
        public ICustomPrefab SetSpawns(WorldEntityInfo entityInfo,
        ...
        public EggGadget CreateCreatureEgg(int requiredAcuSize = 1)
        {
